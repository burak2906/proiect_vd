...
const RepeatOrderPredictionForm = ({ onFormChange }) => {
  const [form, setForm] = useState({
    age: 30,
    order_value: 20.0,
    delivery_fee: 2.5,
    time_taken_to_order: 30,
    order_time: 'Evening',
    day_type: 'Weekday',
    discount_applied: 'No',
  ...
  const submit = (e) => {
    ...
    setLoading(true);
    setResult(null);
    api.post('/predict/repeat-order', form)
  ...
  return (
    <SectionCard
      title="Predict repeat order"
      subtitle="Simulează probabilitatea ca un user să revină"
    >
      <form className="prediction-form" onSubmit={submit}>
        ...
        <div>
          <label>Timp procesare (min)</label>
          <input type="number" name="time_taken_to_order"
            value={form.time_taken_to_order} onChange={handleChange} />
        </div>
        ...
        <div>
          <button className="primary-btn" type="submit" disabled={loading}>
            {loading ? 'Se calculează...' : 'Calculează probabilitate'}
          </button>
        </div>
        ...
        <div className="prediction-result">
          {result.error ? (
            ...
            <>
              <h3>Rezultat</h3>
              <p>
                Probabilitate repeat order:{' '}
                <strong>{(result.probability_repeat * 100).toFixed(1)}%</strong>
              </p>
              <p>
                Clasificare: <strong>{result.prediction}</strong>
              </p>
            </>
          )}
        </div>
