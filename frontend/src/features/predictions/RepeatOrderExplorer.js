...
const COLORS = ['#0f766e', '#0d9488', '#14b8a6', '#2dd4bf', '#1d4ed8', '#3b82f6', '#6366f1'];
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: '1.1rem', color: '#14b8a6' }}>
        {(val * 100).toFixed(1)}% probabilitate repeat
      </div>
    </div>
  );
};

export default function RepeatOrderExplorer({ formValues }) {
  const [varyKey, setVaryKey]   = useState('mood');
    ...
    try {
      const results = await Promise.all(
        currentVary.values.map(async (val) => {
          ...
          const res = await fetch(`${API}/predict/repeat-order`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(body),
          });
          const data = await res.json();
          return { name: val, value: data.probability_repeat };
        })
      );
    ...
    <div style={cardStyle}>
      <div style={headerStyle}>
        <div>
          ...
          <h3 style={titleStyle}>Probabilitate repeat order</h3>
          <p style={subtitleStyle}>Variază un parametru și vezi impactul în timp real</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {loading && <span style={{ fontSize: '0.8rem', color: '#14b8a6' }}>● live</span>}
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={chartData} margin={{ top: 8, right: 16, left: -10, bottom: 4 }}>
            ...
            <YAxis
              ...
              domain={[0, Math.min(1, maxVal * 1.3)]}
            ...
            <Bar dataKey="value" radius={[8, 8, 0, 0]} maxBarSize={64}>
              {chartData.map((entry, i) => (
                <Cell
                  key={i}
                  fill={entry.name === formValues?.[varyKey] ? '#0f766e' : COLORS[i % COLORS.length]}
                  opacity={entry.name === formValues?.[varyKey] ? 1 : 0.65}
                />
              ))}
            </Bar>
          </BarChart>
...
const eyebrowStyle = {
  ...
  textTransform: 'uppercase', color: '#0f766e', marginBottom: 4,
};
